...
ActiveRecord::Base.connection.instance_eval do
  ...
  add_index :user6s, :uid, unique: true

  create_table :user7s do |t|
    t.string :uid, null: false
  end
  add_index :user7s, :uid, unique: true

  create_table :user8s do |t|
    t.string :uuid, null: false
  end
  add_index :user8s, :uuid, unique: true
...
describe Uidable do
  ...
  describe "uniqueness" do
    it "should check uniqueness by default" do
      u = User.create
      u2 = User.create
      u2.uid = u.uid
  ...
  describe "scope" do
    ...
    it "should have the with_uid scope if scope is enabled" do
      User7.must_respond_to :with_uid
    end
    ...
    it "should find a record with uid by the with_uid scope" do
      ...
      User7.create
      User7.create
      found_u = User7.with_uid(u.uid).take
    ...
    it "should have the scope with the changed uid name if uid_name is given and scope is enabled" do
      User8.must_respond_to :with_uuid
      u = User8.create
      User8.create
      User8.create
      User8.create
      found_u = User8.with_uuid(u.uuid).take
      found_u.id.must_equal u.id
